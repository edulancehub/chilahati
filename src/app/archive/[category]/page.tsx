...
export default function ArchiveCategoryPage({ params }: { params: Promise<{ category: string }> }) {
    const { category } = use(params);
    const [mode, setMode] = useState<"loading" | "list" | "subcategories">("loading");
        ...
        return (
            <div className="main-content">
                <header style={{ textAlign: "center", marginBottom: "3rem" }}>
                    <h1>{title}</h1>
                    <p className="subtitle">Select a specific category to explore</p>
                </header>

                <div className="menu-grid">
                    {subTypes.map((sub) => {
                        const slug = String(sub).toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
                        return (
                            <Link key={sub} href={`/archive/${category}/${sub}`} className={`card card-subtype card-${slug}`}>
                                <div className="card-content">
                                    <div className="card-icon"><i className="fas fa-folder-open"></i></div>
                                    <h3 style={{ textTransform: "capitalize" }}>{sub}</h3>
                                    <p style={{ textTransform: "capitalize" }}>View all entries under {sub} {displayCategory}</p>
                                </div>
